...
import { ordersTable, productsTable, newsletterSubscribersTable } from "../db.js";
...
router.get("/", requireAdmin, async (req, res) => {
  try {
    const [orderStats] = await db.select({
      ...
      totalRevenue: sum(ordersTable.totalUsd),
    }).from(ordersTable);
    ...
    const [newsletterStats] = await db.select({
      totalSubscribers: count(newsletterSubscribersTable.id),
    ...
    const lowStockProducts = await db.select().from(productsTable)
      .where(sql`${productsTable.stock} < 10 AND ${productsTable.inStock} = true`)
      .limit(10);
